...
**Factory reset is marker-based.** The daemon writes `/var/lib/nocturne/factory-reset.pending`, clears resettable state, persisted OTA and macOS connector metadata at runtime, and reboots. `nocturne-factory-reset.service` consumes the marker before daemon startup on the next boot as a retry-safe fallback. Preserve the bandaid overlay and `/var/lib/superbird` identity metadata.
