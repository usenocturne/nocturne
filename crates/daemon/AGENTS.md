...
7. **iPhone Address Identity**: A bonded iPhone's BlueZ object path may retain its private address while `Device1.Address` changes to the stable identity. Resolve both forms to the live object, preferring a paired object and then a connected object when stale and current paths share one identity. Exact path matches determine the identity before ranking, so unrelated peers never enter that selection. Use the stable identity for iAP2 ownership, recent pairing timestamps, UI events, and reconnect keys. Both the initial paired snapshot and later `Paired` property changes use one state handler, so a pairing event that races monitor startup still arms direct iAP2 recovery. Incomplete BlueZ metadata is retried without dialing an unclassified peer. A running recovery task promotes its timestamp, deduplication key, and RFCOMM target when the private address resolves to the stable identity. Explicit disconnect cancels and awaits the owned iAP2 transport plus route cleanup before acknowledging, even when BlueZ only owns the companion LE link. Link and session tasks abort on parent-task drop. Recovery uses RFCOMM channel 1 with 2s to 30s backoff, but never arms or continues for a device whose BlueZ `Blocked` property is true. Android and macOS peers remain outside this path.
