...
│   ├── capture.rs          # arecord capture, Opus encoding, VAD-gated silence stop, broadcast channel
│   ├── wakeword.rs         # ONNX-based wake word detector
│   └── wind.rs             # Wind level detector (UI reporting)
├── system/
...
│   └── factory_reset.rs    # Reset marker, runtime state clearing, and reboot coordination
├── http/
...
| Bluetooth SDP/advertising | `bluetooth/mod.rs` | `register_sdp_record()`, `set_advertising()`; startup derives one `Nocturne (XXXX)` identity, applies it to the BlueZ alias before powering the adapter, and passes the same retained name to the BLE AccessorySetupKit advertisement. This ordering prevents BR/EDR scanners from caching the default unsuffixed name. Startup recreates the BlueZ session and retries adapter initialization indefinitely with capped backoff while HTTP and WebSocket stay available. D-Bus ownership races, `NotFound`, `NotReady`, and BlueZ `Busy` must not exit the daemon. |
...
| Audio pipeline | `audio/capture.rs`, `audio/mod.rs`, `audio/dsp.rs`, `audio/wind.rs` | `AudioCapture`, `AudioCommand::Start/Stop`; PCM conversion stays on the capture path while raw four-channel wind analysis runs in a bounded batch worker to protect wake-word real-time headroom. `ARecordPcmConverter` is the shared 4-mic DSP front-end: per-channel 4th-order Butterworth HPF at 100 Hz, wind-aware weighted mix (inverse smoothed incoherent 20-250 Hz power, bounds enforced by pin-and-redistribute normalization), then a 165-tap Kaiser FIR decimator 48 kHz→16 kHz (-76 dB stopband; the old 3-sample boxcar aliased PDM HF noise into the speech band). RNNoise (`nnnoiseless`) runs at 48 kHz on the voice-capture path ONLY — measured on device: it raises speech-band SNR ~9 dB for STT but *hurts* wake word detection in noise (classifiers are trained on noisy-but-natural audio), so `wakeword.rs::fresh_converter` must never enable `with_denoise`. Capture silence auto-stop is VAD-gated: a frame is voice-active iff (RMS ≥ 200 && RNNoise VAD ≥ 0.15) or VAD ≥ 0.75, calibrated from on-device recordings (denoised noise floor RMS 1-16, leakage p90 ≈ 153). Wind analysis consumes raw pre-HPF frames — keep it that way. Front-end costs ~2.3% of one A53 core continuous; +RNNoise ~6% during capture. TODDR_A and TODDR_B both route to the PDM array as two independent capture devices: the wake word listener owns `hw:0,0`, voice capture uses `hw:0,1`, so capture never waits for a device handoff (opening a fresh device still takes ~250 ms — see next). Onset protection: the wake word read loop feeds a shared frame-aligned `PreRollBuffer` (1 s of raw 4-ch bytes); a capture session waits for its first live chunk, then splices in the pre-roll from `Start-time − 300 ms` (covers detection latency + device startup, warms RNNoise) — without this the first command word is lost (`voice capture live preroll_ms=~600 startup_ms=~250` log line shows the recovered window). Wake word transient pause (persist=false, active capture) keeps its arecord draining so pre-roll stays warm and only skips inference; user mute (persist=true) still kills arecord and resets the pre-roll stream. Pre-roll chunks must stay whole-frame aligned and `reset_stream` must run on every arecord (re)spawn — splicing bytes across different arecord runs breaks 16-byte frame alignment and scrambles channels. Design/validation harness: `ab_process_device_recording` ignored test (raw 4ch capture in, per-pipeline PCM + VAD trace out; run the cross-built test binary on device). |
...
| Factory reset | `system/factory_reset.rs`, `http/websocket.rs` | Stage a persistent reset marker, stop runtime services, clear resettable state, and reboot without the removed `uenv` utility |
...
| Phone media progress | `iap2/mod.rs`, `app/msgpack.rs` | Native iAP2 merges sparse elapsed-position and speed deltas, then emits canonical duration, elapsed, and normalized-rate fields. Companion payload normalization must preserve the same nested timing values without unit or casing changes. |
...
Writing-phase OTA recovery is route-only and idempotent. A matching `ota.begin` from the pinned peer updates the active route without replacing the writer. Its matching `ota.package_ready` returns the complete durable offset without spawning another main-payload pull, then reactivates the image delta broker and replays its retained in-flight range request to the replacement route. The range stream ignores the already-consumed prefix of that replay. Reject a different peer, update ID, kind, size, hash, or target version.
...
- **Don't refactor hardcoded paths**: `/etc/nocturne/`, `/etc/superbird`, `/dev/i2c-3` (MFi @ `0x10`), `/dev/misc`, efuse nvmem cells, ALSA `hw:0,0` (wake word) / `hw:0,1` (voice capture), and Amlogic TODDR_A/B `IN 4` are Car Thing filesystem/audio constants
