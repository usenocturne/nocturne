...
use crate::{
    app::AppMessage, error::Result, http::WebSocketServer, iap2::Iap2Connection,
        ...
        if let Some(mut ws_rx) = self.ws_to_app_rx.take() {
            ...
            tokio::spawn(async move {
                while let Some(ws_message) = ws_rx.recv().await {
                    ...
                    let target_peer = target_peer(&ws_message);
                    let conns = connections.lock().await;
                    let mut success_count = 0;
                    for conn in conns.iter() {
                        if target_peer
                            .as_deref()
                            .is_some_and(|peer| conn.device_address().to_string() != peer)
                        {
                            continue;
                        }
                        ...
                        DeviceEvent::PropertyChanged(DeviceProperty::Paired(paired)) => {
                            ...
                            if let Some(ws) = &websocket_server {
                                ws.broadcast_event(
                                    ...
                                    typed_json(BluetoothPairingEvent {
                                        event: Some(
                                            if paired { "paired" } else { "unpaired" }.to_string(),
                                        ),
                                        r#type: None,
                                        device: address.to_string(),
                                    }),
...
<record>
    ...
    async fn handle_new_connection(
        ...
        let connection = Iap2Connection::new(
            device,
            stream,
            inputs.websocket_server,
            inputs.audio_event_rx,
            inputs.audio_cmd_tx,
            inputs.wakeword_pause_tx,
            inputs.ota_cmd_tx,
        )
        .await?;
    ...
    async fn try_iap2_connection(
        ...
        let conn_clone = connection.clone();
        let user_flag = conn_clone.user_disconnect_flag();
        let connections_clone = inputs.connections.clone();
        let ws_server_clone = inputs.websocket_server.clone();
