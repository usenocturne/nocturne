...
export function useCarThingSpotifyIntegration(
  ...
  useEffect(() => {
    if (!carThingStores || !currentPlayback?.item) {
      ...
      shouldInjectPushedArtworkRef.current = false;
      return;
    }
  ...
  useEffect(() => {
    const cleanup = addGlobalWsListener("carthing-artwork", {
      /**
       * @param {{ type?: string, topic?: string, data?: MediaNowPlayingArtworkEvent | MediaNowPlayingArtworkFailedEvent }} data
       */
      onMessage: (data) => {
        ...
        if (
          ...
          if (!isCurrentMediaArtwork(data.data)) {
            return;
          }
          ...
          if (!shouldInjectPushedArtworkRef.current) return;
