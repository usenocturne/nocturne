...
/** @typedef {import("@schema/audio").AudioLevelEvent} AudioLevelEvent */
/** @typedef {import("@schema/audio").AudioRecordStartRequest} AudioRecordStartRequest */
/** @typedef {import("@schema/audio").AudioRecordStopRequest} AudioRecordStopRequest */
/** @typedef {import("@schema/voice").AiResponseEvent} AiResponseEvent */
/** @typedef {import("@schema/voice").AiStateEvent} AiStateEvent */
/** @typedef {import("@schema/voice").AiToolExecutedEvent} AiToolExecutedEvent */
/** @typedef {import("@schema/voice").VoiceTranscriptionEvent} VoiceTranscriptionEvent */
/** @typedef {import("@schema/voice").VoiceWakewordStateEvent} VoiceWakewordStateEvent */
/** @typedef {import("@schema/voice").WakewordPauseRequest} WakewordPauseRequest */
/** @typedef {import("@schema/voice").WakewordResumeRequest} WakewordResumeRequest */
...
const AUDIO_RECORD_START_REQUEST = /** @type {AudioRecordStartRequest} */ {};
const AUDIO_RECORD_STOP_REQUEST = /** @type {AudioRecordStopRequest} */ {};
const WAKEWORD_PAUSE_REQUEST = /** @type {WakewordPauseRequest} */ {};
const WAKEWORD_RESUME_REQUEST = /** @type {WakewordResumeRequest} */ {};
...
class VoiceStore {
  ...
  constructor(
    ...
    this._appReadyCleanup = subscribeAppReadyState(
      action(({ platform }) => {
        this.isMicLocked = isConnectorPlatform(platform);
      }),
    );
  }
  ...
  onTranscription = action((data) => {
    ...
    this.state.asr.isFinal = !!(data.is_final ?? data.isFinal);
    if (this.state.asr.isFinal) {
  ...
  onAIState = action((data) => {
    ...
    if ((isSpeaking || isCleanIdle) && this._pendingVoicePopulation != null) {
      ...
      if (!hasQueueMutation) {
        const intent = deriveVoiceResultIntent(this._toolsExecutedThisSession);
  ...
  onToolExecuted = action((data) => {
    ...
    const toolArguments = data.toolArguments || data.tool_arguments || {};
    if (handler) {
      this._handleVoiceResultTool(data, handler, toolArguments);
      return;
    }

    const intent = deriveSimpleIntent(tool, toolArguments);
    if (intent) {
      this.state.intent = intent;
      this.state.showingVoiceConfirmation = !NO_ICON_INTENTS.has(intent);
      this.state.aiResponse = "";
      if (intent === VOLUME_INTENT) {
        const raw = Number(toolArguments?.volume_percent);
  ...
  retry = action(() => {
    ...
    this._clearCloseTimeout();
    this._startCaptureTimeout();
    sendNocturneWsRequest("audio.record.start", AUDIO_RECORD_START_REQUEST);
  });
  ...
  cancel = action(() => {
    ...
    this._resetVoiceTurnTracking();
    this._clearVoiceResultTimers();
    sendNocturneWsRequest("audio.record.stop", AUDIO_RECORD_STOP_REQUEST).catch(
      () => {},
    );
  ...
  toggleMic = action(() => {
    ...
    if (this.isMicMuted) {
      sendNocturneWsRequest("wakeword.pause", WAKEWORD_PAUSE_REQUEST);
    } else {
      sendNocturneWsRequest("wakeword.resume", WAKEWORD_RESUME_REQUEST);
    }
  });
