...
export type HardwareEventType =
  (typeof HardwareEvent)[keyof typeof HardwareEvent];
...
type HardwareEventListener = (...args: unknown[]) => void;
type LongPressCallback = () => void;
type TimeoutId = ReturnType<typeof setTimeout>;

class LongPressHandler {
  readonly DEFAULT_LONG_PRESS_TIMEOUT: number;
  activeTimeouts: Map<string, TimeoutId>;
  ...
  fireShortPressAndAbortLongPress(
    pressedKey: string,
    shortPressCallback: LongPressCallback,
  ): boolean {
  ...
  clearTimeoutForKey(pressedKey: string) {
    if (this.activeTimeouts.has(pressedKey)) {
...
export class HardwareEvents {
  listeners: Map<HardwareEventType, HardwareEventListener>;
  keysDown: Set<string>;
