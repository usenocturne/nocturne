type HexColorPalette = string[];
type RgbTuple = [number, number, number];

export function extractColorsFromImage(
  imageUrl: string,
): Promise<HexColorPalette> {
...
export function getBackgroundColor(imageBase64: string): Promise<RgbTuple> {
  return extractColorsFromImage(`data:image/jpeg;base64,${imageBase64}`)
    .then((colors) => {
      ...
      if (darkColor?.startsWith("#")) {
        ...
        return [r, g, b] satisfies RgbTuple;
      }
      return [26, 26, 26] satisfies RgbTuple;
    })
    .catch(() => [26, 26, 26] satisfies RgbTuple);
}
