...
- **Preset context identity:** Normalize preset and tracklist contexts through `src/utils/spotifyContext.ts`. Liked Songs aliases share the `spotify:collection:your-music` sentinel, and legacy personalized playlist URIs must be canonicalized before metadata lookup, persistence, or playback.
