...
const limitColorBrightness = (hexColor: string) => {
...
const needsDarkGradientEnhancement = (colors: string[]) => {
...
const createEnhancedDarkGradient = (
  colors: string[],
...
export function useGradientTransition(activeSection) {
  ...
  const updateGradientColors = useCallback(
    async (
      imageUrlOrColors: string | string[] | null,
      imageSection: string | null = null,
    ) => {
