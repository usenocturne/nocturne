...
const ROTOR_BASE_STYLE: CSSProperties = {
  position: "absolute",
  top: "50%",
  left: "50%",
  width: "160vmax",
...
export default function VoiceBorder({
  active,
  intensity,
  phase,
  micLevelRef,
  ...
  const bloomWrapRef = useRef<HTMLDivElement | null>(null);
  const highlightWrapRef = useRef<HTMLDivElement | null>(null);
  const glowRotorRef = useRef<HTMLDivElement | null>(null);
  const bloomRotorRef = useRef<HTMLDivElement | null>(null);
  const highlightRotorRef = useRef<HTMLDivElement | null>(null);
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      if (!rootRef.current || lastTimeRef.current === null) return;
