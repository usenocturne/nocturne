...
Spotify local files are identified by their `spotify:local:` item URI because player-state responses do not reliably include `is_local`. Never send `spotify:localfileimage:` through `spotify.image.fetch`; show the not-playing asset until correlated phone artwork arrives. Periodic player-state responses can contain blank artist names, so preserve named artists and pushed blob artwork only for the exact same local URI or while a same-title pending placeholder resolves to that URI, then derive a missing artist from the local URI when no richer metadata exists. Keep the Spotify like button visible for local files; only phone media replaces that leading control with an empty spacer.
