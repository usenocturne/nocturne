...
`SpotifyImage` must never wear a previous item's artwork indefinitely: when its image URL changes, the old image may persist only through a short grace window (`STALE_ARTWORK_GRACE_MS`) while the new remote image loads, and a skipped fetch (`disableSpotifyFetch` / the now-playing window) swaps to the fallback immediately. Without this, a slow or failing `spotify.image.fetch` after a source switch keeps the previous app's artwork (e.g. a stale YouTube thumbnail) on a Spotify track for minutes.
