...
- **Pairing preserves its owner:** A Bluetooth PIN request is a visual overlay, not a replacement system screen. Keep the disconnected `NetworkScreen` and its mounted `BluetoothDevices` subtree alive behind the PIN until an iAP2 `bluetooth.connection` event changes connection state. Pairing PIN aliases must select the first string value, not merely the first defined value, because daemon legacy PIN events can contain `pin: null` with the usable value in `pincode`. Canonical `bluetooth.pairing` `event: "paired"`, legacy `type: "pairing_succeeded"`, and agent cancellation clear only the PIN state. Generic BlueZ `bluetooth.device` connectivity is not iAP2 readiness.
