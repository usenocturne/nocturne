...
- **Wake-word sensitivity**: the daemon requires two consecutive per-keyword scores across two 80 ms inference windows, both at least the support threshold and one at least the peak threshold before emitting a candidate. `WAKEWORD_THRESHOLD` controls the peak, `WAKEWORD_SUPPORT_THRESHOLD` controls supporting scores, and `WAKEWORD_PLAYBACK_THRESHOLD` replaces the peak inside the detector while media is playing and defaults to the normal peak. A candidate remains globally pending until the main loop pauses the listener after acceptance or explicitly rejects and resets it because no app session is ready. Keep the playback state parser compatible with Pascal case, snake case, and companion camel case attribute names.
...
- **Phone app launch preference**: The main UI exposes Open Phone App and Mockingbird exposes Auto launch app, enabled by default. The daemon persists `foreground` in `/var/lib/nocturne/app-launch.json` through `device.appLaunch.get/set`; UI settings only change after acknowledgement. Disabled suppresses iAP2 foreground launch requests while retaining External Accessory sessions. Android reads the same preference over MessagePack before opening its activity and always retains its companion connection service. Android app changes ship from the separate mobile repository.
